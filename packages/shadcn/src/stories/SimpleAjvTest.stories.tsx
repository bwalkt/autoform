  ...
  render: () => {
    ...
    React.useEffect(() => {
      try {
        ...
        setResult(
          `✅ Success! parseSchema returned ${JSON.stringify(parsed, null, 2)}`,
        );
      } catch (error) {
    ...
    React.useEffect(() => {
      async function testCoreParseSchema() {
        try {
          ...
          console.log(
            "Provider.parseSchema before call:",
          ...
          setResult(
            `✅ Core parseSchema works! Result: ${JSON.stringify(result, null, 2)}`,
          );
        } catch (error) {
          console.error("Error in core parseSchema test:", error);
          setResult(
            `❌ Core parseSchema error: ${error.message}\n\nStack: ${error.stack}`,
          );
        }
      }
    ...
    return (
      <div className="p-4">
        <h3 className="text-lg font-medium mb-4">
          Core parseSchema Function Test
        </h3>
        <div className="p-4 bg-gray-100 rounded">
          <pre className="text-sm whitespace-pre-wrap">{result}</pre>
        </div>
        <div className="mt-4 text-sm text-gray-600">
          This tests the exact same call that AutoForm makes internally.
        </div>
      </div>
    );
  },
